...
void f2f_icp(pcl::PointCloud<PointType>::Ptr scanCloudCurrent,pcl::PointCloud<PointType>::Ptr scanCloudLast,const geometry_msgs::PoseStamped &correctPoseLast)
{
    ...
    *aligned_f2f = *scanCloudLast;
    ...
    predictPoseCurrent.header.frame_id = "my_frame";//Need to define a world and body frame in rviz
    predictPoseCurrent.header.stamp = ros::Time::now();//Need to use the timestamp of the current scan cloud
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber correctPoseLast = n.subscribe<geometry_msgs::PoseStamped>("/pose", 10, correctPoseLastHandler);
    ros::Subscriber scanCloud = n.subscribe<sensor_msgs::PointCloud2>("/lidar_points", 10, scanCloudHandler);
