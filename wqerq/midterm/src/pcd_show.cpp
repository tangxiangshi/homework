...
//base_link
Eigen::Vector3d lidar_imu_t(0.46,0,3.46); //这个是怎样确定的呢
Eigen::Quaterniond lidar_imu_r(-0.0051505,0.018102,-0.019207,0.99964); // 这个是怎样确定的
...
void show_map(pcl::PointCloud<PointType>::Ptr map_cloud)
{
  ...
  sensor_msgs::PointCloud2 output,output1; // output 与 output1都是什么？
  std::vector<int> map_indices;

  pcl::io::loadPCDFile("/home/tangxiangshi/Desktop/ITRI/map.pcd", *map_cloud);

  pcl::removeNaNFromPointCloud(*map_cloud, *map_cloud, map_indices); // map_indices 做什么用？ 我怎么没看懂
...
//  pass.setFilterLimitsNegative(false);
  ...
  pass.setFilterLimits(init_point_last(0)-80,init_point_last(0)+80); // 用init_point_last 位置似乎不对啊，这样的init_point位置还是lidar坐标系的位置
  pass.filter(*map_icp);
...
//  pass.filter(*map_icp);
  ...
  map_part_pub.publish(output1); // 发布局部icp


  pcl::toROSMsg(*map_cloud, output);
  output.header.stamp = ros::Time::now();
  output.header.frame_id = "/map";
  map_pub.publish(output); // 发布全局icp
}
...
void solve_icp(pcl::PointCloud<PointType>::Ptr scan_cloud, pcl::PointCloud<PointType>::Ptr map_cloud, Eigen::Vector3d init_point_last) // 这里面的map_cloud 是做什么用的呢
{
...
/*********************compute attitude every 15 degree******************/
  for(int i = 0 ;i < (360/perDegree); i++)
  {
    ...
    */
    for(int j=0; j < cloud_icp->points.size(); j++)
    {
      ...
      //先旋轉載平移
      trans << scan_cloud->points[j].x ,scan_cloud->points[j].y ,scan_cloud->points[j].z ; // coordinate in lidar frame 这里第一次不是什么都没有么，我觉得这里有问题 ？？ //这里是不是应该是cloud_icp->points, 已解决，其实是没有问题的，这里面要看solve icp 传进来的参数
      ro = rotation; 
      trans = ro * trans;
      cloud_icp->points[j].x = trans(0) + init_point_last(0);  // 这里不是很懂
  ...
  //車子r 和 t
  inital_guess.pose.position.x = init_point_last(0) + trans_(0); // 这里的trans_是什么？ initial_guess 有什么用？
...
void f2f_icp(pcl::PointCloud<PointType>::Ptr scanCloudCurrent,pcl::PointCloud<PointType>::Ptr scanCloudLast,geometry_msgs::PoseStamped &inital_guess)
{
    ...
    X_q_hat = X_q_hat.toRotationMatrix() * best_matrix.toRotationMatrix();  // 这个是什么意思
    X_r_hat = X_r_hat + X_q_hat * best_trslation;
    eulerAngle = X_q_hat.toRotationMatrix().eulerAngles(2,1,0); // ZYX順序，即roll pitch yaw順序
...
int main (int argc, char **argv)
{
  ...
  bool flag_csv =true , flag_up = true; // 这两个做什么用的
  //std::cout<<"here1"<<std::endl;
  ros::Rate loop_rate(10);
  int count = 0; // 这个又是做什么用的
  while(ros::ok)
  {
    //std::cout<<"here2"<<std::endl;
    show_map(map_cloud);  // 这里的 map_cloud 在哪里
    //std::cout<<"here3"<<std::endl;

    if(!scanCloudBuf.empty())
    {
      pcl::fromROSMsg(*scanCloudBuf.front(), *scanCloudCurrent); // 这样的话 会不会每次都是第一次进入的点云啊

      //std::cout<<scanCloudLast->points.size()<<std::endl;
      std::cout<<"here4"<<std::endl;
      if((scanCloudCurrent->points.size()>0) && (icp_flag == true))
        {
          std::vector<int> scan_indices; // 这个是做什么的？
...
//        pass.filter(*scanCloudCurrent);
        if(flag_up == true) // 这里的flag_up是做什么用的？
        {
          flag_up = false;
        }
        else
      ...
      std::cout<<"!scanCloudBuf.empty()"<<!scanCloudBuf.empty()<<std::endl;  // !scanCloudBuf.empty()是什么意思
      ros::spinOnce();
      loop_rate.sleep();
    }
  return 0;
