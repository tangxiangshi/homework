...
int main(int argc, char **argv)
{
	...
	while(ros::ok()) {
		 ...
		 */
		float a = 0; // initiallize the accelerometer 
		a = ( u - k * x1.current - b * x2.current ) / M ; // accelerometer calculation 
		x2.current += a * dt ; // current velocity calculation 
		x1.current += x2.current * dt ; // current position calculation			
